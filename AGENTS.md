...
**IMPORTANT :** ne retirez pas `@mister-guiiug/dev-wpa-config` de `package.json` ni les configs qui le réexportent (`eslint.config.js`, `prettier.config.js`, `lint-staged.config.js`, `vitest.config.ts`, `tsconfig.*.json`).
...
- Stack : React 19 + Vite 8 (Rolldown) + Vitest 4 + TypeScript 6 strict, configs mutualisées via `@mister-guiiug/dev-wpa-config`.
